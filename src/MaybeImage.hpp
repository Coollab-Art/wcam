...
using MaybeImage = std::variant<
    std::shared_ptr<img::Image const>, // TODO store a shared ptr so that we know if ppl are holding ref on that or not, to know when we can switch to a new image?
